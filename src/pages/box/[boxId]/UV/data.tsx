...
export const getServerSideProps = (async(context:any)=>{
    ...
    .then((res)=> res.json());
    const sensorId = getSensorId(sensors, "UV Sensor");
    const data = await fetch(`https://minibrightbox.fly.dev/api/sensors?id=${sensorId}&type=uv`)
    return{
        props: {
...
export default function Page({data}: InferGetServerSidePropsType<typeof getServerSideProps>) {
    return (
      <>
    <LineChart width={"100%"} height={"100%"} color={"#650cb3"} name={"UV"} unit={'nm'} data={transformSensorData(data)} datakey={"value"} domain={[450, 600]}/>
      </>
    )
}